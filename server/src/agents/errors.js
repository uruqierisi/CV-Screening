 ...
 * which is phase 2b and imports nothing from here. These are different animals:
 * they are thrown by pure functions, they are never retryable, and they must be
 * importable by code that has no idea an SDK exists. Hence a separate module at
 * the root of the agent layer rather than a `client/` file 2a would otherwise
 * have to create early.
 *
 * Every error carries:
 ...
 *   over the same input produces the same failure. The one exception is
 *   `SummaryContainsScoreError`, where the input a retry would change is not the
 *   argument but the generation that produced it - see its own note;
 ...
 *
 * **The one retryable error in this file.** Every other member fails the same way
 * however many times it is re-run, because the input is fixed. Here the input a
 * retry changes is the *generation*, not the argument: this is a semantic
